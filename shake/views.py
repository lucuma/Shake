...
from .templates import default_render
...
def show_template(request, render, template, **kwargs):
    """A really simple view who render directly a template.

    :param render: The template renderer to use.

    :param kwargs: Values to add to the template context.
    """
...
def check_if_exist(request, model, column_name):
    """A view that check if a row in a model already exists (eg. a username).
    This view is intended to be called using AJAX.
