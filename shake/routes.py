...
def get_converter(map, name, args):
    ...
    if args:
        ...
        args, kwargs = eval(u'(lambda *a, **kw: (a, kw))(%s)'
            % args, {}, storage)
    else:
        args = ()
        kwargs = {}
...
class Submount(RuleFactory):
    """Like `Subdomain` but prefixes the URL rule with a given string::
        ...
        url_map = Map([
            ...
            Submount('/blog', [
                Rule('/', 'blog/index'),
                Rule('/entry/<entry_slug>', 'blog/show')
            ])
        ])
...
class RuleTemplate(object):
    ...
    Here a small example for such a rule template::
        ...
        from shake import Map, Rule, RuleTemplate
        
        resource = RuleTemplate([
            Rule('/$name/', '$name.list'),
            Rule('/$name/<int:id>', '$name.show')
        ])
...
class Rule(RuleFactory):
    ...
    Note that besides the rule-string and endpoint all arguments *must* be
    keyword arguments in order to not break the application on Shake upgrades.
    ...
    :param defaults:
        ...
        This is a bit tricky but useful if you want to have unique URLs::
            ...
            url_map = Map([
                Rule('/all/', 'all_entries', defaults={'page': 1}),
                Rule('/all/page/<int:page>', 'all_entries')
            ])
    ...
    :param subdomain:
        ...
        and all subdomains are forwarded to your application::
            ...
            url_map = Map([
                Rule('/', 'user/homepage', subdomain='<username>'),
                Rule('/stats', 'user/stats', subdomain='<username>')
            ])
    
    :param methods:
        A sequence of http methods this rule applies to. If not specified, all
        methods are allowed. For example this can be useful if you want
        different endpoints for `POST` and `GET`. If methods are defined and
        the path matches but the method matched against is not in this list or
        in the list of another rule for that path the error raised is of the
        type `MethodNotAllowed` rather than `NotFound`. If `GET` is present in
        the list of methods and `HEAD` is not, `HEAD` is added automatically.
    ...
    :param build_only:
        ...
        or directory that are not handled by the WSGI application
        (like static data).
    ...
    :param name:
        Optional, more or less static, user-friendly way to point to a Rule.
        If used and the endpoint changes location, only the Rule definition
        need to be changed, instead of all url_for calls.
    
    """
    
    def __init__(self, string, endpoint=None, defaults=None, subdomain=None,
      methods=None, build_only=False, strict_slashes=None, redirect_to=None,
      name=None):
    ...
    def empty(self):
        ...
        return Rule(self.rule, self.endpoint, defaults, self.subdomain,
            self.methods, self.build_only, self.strict_slashes,
            self.redirect_to, self.name)
    ...
    def match(self, path):
        ...
        if not self.build_only:
            ...
            if m is not None:
                groups = m.groupdict()
                # we have a directory like part of the url without a trailing
        ...
        if append_unknown:
            ...
            if query_vars:
                url += '?' + url_encode(query_vars, self.map.charset,
                    sort=self.map.sort_parameters, key=self.map.sort_key)
...
class IntegerConverter(NumberConverter):
    ...
    :param fixed_digits: the number of fixed digits in the URL. If you set
        this to ``4`` for example, the application will only match if the
...
class Map(object):
    ...
    :param redirect_defaults: This will redirect to the default rule if it
        wasn't visited that way. This helps creating unique URLs.
    
    :param converters: A dict of converters that adds additional converters
       to the list of converters. If you redefine one converter this will
    ...
    def __init__(self, rules=None, default_subdomain='', charset='utf-8',
      strict_slashes=True, redirect_defaults=True, converters=None,
      sort_parameters=False, sort_key=None, encoding_errors='ignore'):
        self._rules = []
    ...
    def is_endpoint_expecting(self, endpoint, *arguments):
        ...
        :param arguments: this function accepts one or more arguments
            as positional arguments. Each one of them is checked.
        """
        self.update()
    ...
    def add(self, rulefactory):
        ...
        for rule in rulefactory.get_rules(self):
            ...
            if rule.name:
                self._rules_by_name.setdefault(rule.name, []).append(rule)
        self._remap = True
    ...
    def bind(self, server_name, script_name=None, subdomain=None,
        ...
        redirect exceptions raised by Shake will contain the full canonical
        URL.
    ...
    def bind_to_environ(self, environ, server_name=None, subdomain=None):
        ...
        provide it, Shake will use `SERVER_NAME` and `SERVER_PORT`
        as used `server_name` with disabled subdomain
        feature.
        ...
        assert isinstance(environ, dict), ('%r is not a WSGI environment '
            '(has to be a dict)' % environ.__name__)
        
        if server_name is None:
            if 'HTTP_HOST' in environ:
                server_name = environ['HTTP_HOST'].lower()
            else:
                server_name = environ['SERVER_NAME'].lower()
                if ((environ['wsgi.url_scheme'], environ['SERVER_PORT']) not
                        in (('https', '443'), ('http', '80'))):
                    server_name += ':' + environ['SERVER_PORT']
        elif subdomain is None:
            if 'HTTP_HOST' in environ:
                wsgi_server_name = environ.get('HTTP_HOST').lower()
            else:
                wsgi_server_name = environ.get('SERVER_NAME').lower()
                if (environ['wsgi.url_scheme'], environ['SERVER_PORT']) not \
                   in (('https', '443'), ('http', '80')):
            ...
            if cur_server_name[offset:] != real_server_name:
                raise ValueError('the server name provided (%r) does not '
                    'match the server name from the WSGI environment (%r)' %
...
class MapAdapter(object):
    ...
    def match(self, path_info=None, method=None, return_rule=False,
        ...
        - you receive a `RequestRedirect` exception with a `new_url`
          ...
          Shake requests from your WSGI application. This is for example the
        ...
        ...     Rule('/downloads/<int:id>', 'downloads/show')
        ... ])
    ...
    def _partial_build(self, endpoint, values, method, append_unknown):
        ...
        for rule in self.map._rules_by_endpoint.get(endpoint, ()):
            if rule.suitable_for(values, method):
                ...
                if rv is not None:
                    return rv
        for rule in self.map._rules_by_name.get(endpoint, ()):
            if rule.suitable_for(values, method):
                rv = rule.build(values, append_unknown)
                if rv is not None:
                    return rv
    ...
    def build(self, endpoint, values=None, method=None, force_external=False,
        ...
        :param append_unknown: unknown parameters are appended to the generated
            URL as query string argument. Disable this if you want the
            builder to ignore those.
